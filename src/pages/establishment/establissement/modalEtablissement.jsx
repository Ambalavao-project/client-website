...
const ModalEtablissement = ({ isOpen, onClose, etablissement }) => {
    ...
    const { data, isLoading } = UseFetchDataById("get_establishment_by_id", etablissement, isOpen)
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-modal-overlay">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl p-6 md:p-10 relative overflow-y-auto max-h-[90vh] animate-modal-content">
                <button className="absolute top-5 right-6 text-3xl text-gray-400 hover:text-pink-600 transition-colors z-10" onClick={onClose}>
                    &times;
                </button>
                ...
                <h2 className="text-2xl sm:text-4xl font-extrabold text-gray-950 text-center mb-8 pb-4 border-b border-gray-100 flex items-center justify-center gap-3">
                    <FaBuilding className="text-pink-600" size={30} />
                        ...
                        <div className="flex flex-col items-start gap-4 mt-6 w-full px-2 border-t border-gray-100 pt-6">
                            ...
                            <div className="flex items-center gap-3 group">
                                <div className="flex-shrink-0 w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center group-hover:bg-blue-100 transition-colors">
                                    ...
                                    <FaLandmark className="text-blue-600" size={14} />
                                </div>
                                <div className="flex flex-col items-start">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Secteur / Type</span>
                                    <p className="text-sm text-gray-800 font-extrabold leading-none">
                                        {data?.types || "Non spécifié"}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 group">
                ...
                <div className="mt-12 flex justify-center">
                    <button onClick={onClose} className="bg-gray-900 hover:bg-pink-600 text-white font-bold px-12 py-3.5 rounded-2xl transition-all duration-300 shadow-xl ">Fermer la Fiche</button>
                </div>
            </div>
        </div>
    )
